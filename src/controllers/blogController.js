...
const createBlog = async (req, res) => {
    try {
        const data = req.body
        if (!validRequestBody(data)) return res.status(400).send({ status: false, msg: "Please provide something to create blog" })
        ...
        if (!validObjectId(authorId)) return res.status(400).send({ status: false, msg: `${authorId} -> Author Id should be valid` })
        const validAuthorId = await authorModel.findById(authorId)
        if (!validAuthorId) return res.status(400).send({ status: false, msg: "Author Does not exist" })
        ...
        isPublished ? result.publishedAt = new Date() : result.publishedAt = null
        createdBlog.save()
        return res.status(201).send({ status: true, msg: "New Blog Created Successfully", data: createdBlog })
    }
    catch (error) {
...
const getblog = async (req, res) => {
    try {
        ...
        if (getBlogs.length == 0) return res.status(400).send({ status: false, msg: "No blog found" })
        return res.status(200).send({ status: true, msg: "Blog list", data: getBlogs })
...
let updateBlog = async (req, res) => {
    try {
        ...
        if (!validRequestBody(data)) return res.status(400).send({ status: false, msg: "Please provide something to Update blog" })

        if (!validObjectId(blogId)) return res.status(400).send({ status: false, msg: "BlogId is not valid" })
...
let deleteBlog = async (req, res) => {
    try {
        ...
        if (!validObjectId(blogId)) return res.status(400).send({ status: false, msg: `${blogId} is not a valid Blog Id` })
        if (!validObjectId(authorIdFromToken)) return res.status(400).send({ status: false, msg: `${authorIdFromToken} is not a valid token Id` })
        ...
        if (blog.authorId.toString() !== authorIdFromToken) return res.status(401).send({ status: false, msg: "Unauthorised access! Owner info does not match" })
...
const deleteBlogs = async function (req, res) {
    try {
        ...
        if (!validObjectId(authorIdFromToken)) return res.status(400).send({ status: false, msg: 'No query param received,aborting delete operation' })
        ...
        if (blogs.length === 0) return res.status(404).send({ staus: false, msg: "No matching blogs found" })

        const idOfBlogsToDelete = blogs.map(blogs => {if (blogs.authorId.toString() === authorIdFromToken) return blogs._id})
        if (idOfBlogsToDelete.length === 0) return res.status(404).send({ status: false, msg: "No blogs found" })
